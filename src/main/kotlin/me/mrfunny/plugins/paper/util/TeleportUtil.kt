...
object TeleportUtil {
    ...
    fun pullEntityToLocation(entity: Player, loc: Location) {
        ...
        val vX: Double = (1.0 + 0.07 * distance) * (loc.x - entityLoc.x) / distance
        val vY: Double = (1.0 + 0.03 * distance) * (loc.y - entityLoc.y) / distance - 0.5 * gravity * distance * 2
        val vZ: Double = (1.0 + 0.07 * distance) * (loc.z - entityLoc.z) / distance
