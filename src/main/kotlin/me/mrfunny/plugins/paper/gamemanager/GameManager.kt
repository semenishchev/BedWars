...
class GameManager(val plugin: BedWars, version: Double) {
    ...
    val scoreboard: JScoreboard = JScoreboard(JScoreboardOptions("&c&lBedWars", JScoreboardTabHealthStyle.NONE, true))
    ...
    init {
        ...
        configurationManager.loadWorld(configurationManager.randomMapName()) { world: GameWorld ->
            ...
            for(island: Island in world.islands){
                ...
                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "team modify ${island.color.formattedName()}  collisionRule pushOtherTeams")
            }
        }
        ...
        when(value){
            GameState.LOBBY -> {
                if (isStarting()) {
                    ...
                    Reflect.resetField(this, "gameStartingTask")
...
//                plugin.sendPluginMessage("selector:bedwars","UpdateState", "ACTIVE")
                ...
                Bukkit.getScheduler().runTaskLater(plugin, { -> startingPowers.forEach { it.disable() } }, 20 * 30)
            ...
            GameState.WON -> {
                ...
                if (!finalIsland.isPresent) {
                    ...
                    Bukkit.broadcastMessage("&8${StringUtils.center("", 40, "▬")}&8".colorize())
                    Bukkit.broadcastMessage("&8${StringUtils.center("", 40)}&8".colorize())
...
//                        plugin.sendPluginMessage("coins:in", element, "7wZk8c5J3mgvFgUbK", element.uniqueId, "silver", 50 + (island.calculateStat() * 10)
                    ...
                    Bukkit.broadcastMessage("&8${StringUtils.center("", 40,
                        " ")}&8".colorize())
                    Bukkit.broadcastMessage("&8${StringUtils.center("", 40, "▬")}&8".colorize())
                    Bukkit.getOnlinePlayers().forEach {
                        it.sendMessage("&8${StringUtils.center("Round summary", 30, "▬")}&8".colorize())
                        it.sendMessage("")
                        ...
                        it.sendMessage("")
                        it.sendMessage("&8${StringUtils.center("", 40, "▬")}&8".colorize())
...
//                builder.append(" ${if (island.alivePlayerCount() == 0 && !island.isBedPlaced()) "" else "&r&${island.color.getChatColor().char}"}${island.color.formattedName()}")
        ...
        lines.add("")
        lines.add("${ChatColor.RED}play.rubynex.net")
...
//        }
    ...
    fun isStarting(): Boolean{
        return this::gameStartingTask.isInitialized
    }
