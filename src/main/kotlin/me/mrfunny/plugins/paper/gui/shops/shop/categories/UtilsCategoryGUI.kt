...
class UtilsCategoryGUI(gameManager: GameManager, data: PlayerData): PrimaryShopGUI(gameManager, data.player, arrayOf(
    ...
    ShopItem(20, GeneratorType.IRON.getMaterial(), ShopCategory.UTILITY, ItemBuilder(Material.SNOWBALL).setName(gameManager.itemsLocalization.get().getString("building-snowball.${if(data.isRussian()) "ru" else "en"}.name")!!).toItemStack(), *gameManager.itemsLocalization.get()
    ...
    ShopItem(15, GeneratorType.GOLD.getMaterial(), ShopCategory.UTILITY, ItemBuilder(Material.FISHING_ROD).setUnbreakable(false).setName(gameManager.itemsLocalization.get().getString("grappling-hook.${if(data.isRussian()) "ru" else "en"}.name")!!).toItemStack(), *gameManager.itemsLocalization.get()
