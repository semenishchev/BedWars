package me.mrfunny.bedwars.worlds
...
import me.mrfunny.bedwars.game.GameEvent
import me.mrfunny.bedwars.messages.MessagesManager.Companion.message
...
class GameWorld(var name: String, val gameManager: GameManager) {
    ...
    lateinit var generators: ArrayList<Generator>

    lateinit var lobbyPosition: Location
    private lateinit var destinationWorldFolder: File

    var maxTeamSize: Int = 0
    var maxTeams: Int = 0

    private var diamondTier: GeneratorTier = GeneratorTier.ONE
    private var emeraldTier: GeneratorTier = GeneratorTier.ONE
...
//        println("\n\n\n${gameManager.gameConfig.get().isConfigurationSection("max-teams")} ${gameManager.gameConfig.get().isConfigurationSection("max-team-size")}\n\n\n")
    ...
    fun getIslandOf(player: Player): Island {
        return getIslandForPlayer(player)!!
    }

    fun getIslandOf(player: PlayerData): Island {
        return player.assignedIsland
    }
