...
class PlayerManager(private val gameManager: GameManager) {
    ...
    fun playerTeamSelector(player: Player) {
        ...
        player.inventory.setItem(4, ItemBuilder(woolMaterial).setName("&aTeam Selector").toItemStack())
        player.inventory.setItem(7, ItemBuilder(Material.BLAZE_POWDER).setName("&aStart power selector").toItemStack())
